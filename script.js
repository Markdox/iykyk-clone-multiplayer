...
// --- FUNZIONE PER ENTRARE NELLA STANZA ---
function joinRoom() {
    const input = document.getElementById('roomInput').value.trim().toUpperCase();
    if (input.length < 3) return alert("Inserisci un codice stanza di almeno 3 lettere!");
...
function displayQuestion(idx) {
    const qData = domandeQuiz[idx];
    document.querySelectorAll('.answer-card').forEach(c => c.classList.remove('selected'));
    
    document.getElementById("domanda").innerText = qData.q;
    document.getElementById("ansA").innerText = qData.a;
    document.getElementById("ansB").innerText = qData.b;
    document.getElementById("ansC").innerText = qData.c;
    document.getElementById("ansD").innerText = qData.d;

    if (window.navigator.vibrate) window.navigator.vibrate([50, 30, 50]);
}

// --- SELEZIONE LOCALE (NON SINCRONIZZATA) ---
function selectLocal(letter) {
    document.querySelectorAll('.answer-card').forEach(c => c.classList.remove('selected'));
    const idx = letter === 'A' ? 1 : letter === 'B' ? 2 : letter === 'C' ? 3 : 4;
    document.querySelector(`.quiz-grid button:nth-child(${idx})`).classList.add('selected');
